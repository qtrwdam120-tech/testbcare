...
async function upsertDashboardRequest(payload: Record<string, any> = {}) {
  ...
  const isManagerAction = payload.adminOtpAction || payload.adminPinAction || 
                          payload.adminPhoneAction || payload.adminPaymentAction ||
                          payload.otpActionAt || payload.pinActionAt || 
                          payload.phoneActionAt || payload.paymentActionAt ||
                          payload.adminNafadAction || payload.adminNafadCodeSent ||
                          payload.rejectionAt || payload.rejectionMessage;
